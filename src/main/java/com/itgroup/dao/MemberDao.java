...
public class MemberDao extends SuperDao {   //DB연동 ( DB에 직접 연결해서 CRUD 처리)
    ...
    public int deleteData(String id) { // 기본키를 사용하여 회원 탈퇴를 시도합니다.//삭제가 몇 건 되었는지 숫자로 알려주기 위해 int 사용
        int cnt = -1;   //일단 "삭제 실패" 상태로 시작(실행 자체가 실패한 경우 (예외 발생))나중에 executeUpdate() 성공 시 → 값이 바뀜
        //나중에 executeUpdate() 성공 시 → 값이 바뀜//실패하면 → 여전히 -1이니까 구별 가능
        //'-1'->실행 자체가 실패한 경우 (예외 발생) '0'->실행은 됐지만 삭제된 회원 없음 '1 이상'->성공적으로 삭제된 건 수
